 ...
 * 
 * Represents an exam schedule entry or holiday for a specific class/section.
 ...
 * @property {string} section - Section for which exam is scheduled. Use "All" for all sections or specific section like "A", "B", "C"
...
/**
 * Get routines for a specific section
 * 
 * @param {string} section - Section identifier (e.g., "A", "B", "All")
 * @returns {Promise<ExamRoutine[]>} Routines for the specified section
 */
export async function getRoutinesBySection(section: string): Promise<ExamRoutine[]> {
  try {
    const routines = await loadExamRoutines();
    return routines.filter(routine => 
      routine.section === section || routine.section === "All"
    );
  } catch (error) {
    console.error('[examRoutinesHelper] Error getting routines by section:', error);
    return [];
  }
}

/**
 * Get routines for a specific class and section
 * 
 * @param {string} className - Class identifier (e.g., "10", "11")
 * @param {string} section - Section identifier (e.g., "A", "B")
 * @returns {Promise<ExamRoutine[]>} Routines matching both class and section
 */
export async function getRoutinesByClassAndSection(
  className: string,
  section: string
): Promise<ExamRoutine[]> {
  try {
    ...
    console.error('[examRoutinesHelper] Error getting routines by class and section:', error);
    return [];
  }
}
